...
namespace P2PNet
{
    ...
    public class Channel : IDisposable
    {
        ...
        /// </summary>
        public ConcurrentQueue<(IPEndPoint, Exception, byte[])> UnRensponsiveIPs => _unRensponsiveIPs;

        ConcurrentQueue<(IPEndPoint, byte[])> _readQueue;
        ConcurrentQueue<(IPEndPoint, byte[])> _sendQueue;
        ConcurrentQueue<(IPEndPoint, Exception, byte[])> _unRensponsiveIPs;
        ...
        void ProcessSend()
        {
            while (_sendQueue.TryDequeue(out (IPEndPoint ip, byte[] bytes) packet))
            {
                _vehicle.Send(packet.ip, packet.bytes, (e) =>
                {
                    if (e != null)
                    {
                        _unRensponsiveIPs.Enqueue((packet.ip, e, packet.bytes));
                    }
                });
            }
        }
