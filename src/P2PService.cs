...
namespace P2PNet
{
    ...
    public class P2PService
    {
        ...
        /// </summary>
        public event Action<IPEndPoint, Exception, Packet> OnFailToSend;
        ...
        /// </summary>
        public IReadOnlyCollection<IReadOnlyClient> Clients => _clients.Values.ToArray();
        ...
        /// </summary>
        public IReadOnlyClient GetClient(Packet paquet) => GetClient(paquet.ID);
        /// <summary>
        ...
        void Broadcast(Channel channel)
        {
            ...
            while (channel.UnRensponsiveIPs.TryDequeue(out (IPEndPoint ip, Exception e, byte[] bytes) response))
            {
                var targetClient = GetClient(response.ip);

                if (SelfClient.Host)
                {
                    if (ConnectedToServer && targetClient != null && !targetClient.Host)
                    {
                        DisconnectClientAsHost(targetClient.ID, DisconnectInfo.UnResponsive);
                    }
                }
                else
                {
                ...
                OnFailToSend?.Invoke(response.ip, response.e, Serializer.DeserializeItem<Packet>(response.bytes));
            }
